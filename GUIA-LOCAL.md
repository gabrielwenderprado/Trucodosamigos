...
Esta versão standalone já tem ranking, semanas, prémios, roleta e sorteio automático de duas equipas. A fotografia do truco fica em `assets/montagem_truco.png` e aparece como destaque no topo. O sorteio usa sete jogadores, formando um time com três e outro com quatro. Para publicar no GitHub Pages, mantenha a pasta `assets` junto do `index.html` e do `data.js`.
...
O ranking geral usa os pontos definidos no array `players`. A roleta usa automaticamente os sete jogadores e sorteia duas equipas, com três jogadores no primeiro time e quatro no segundo. O campeão de cada semana é calculado automaticamente como o time com maior pontuação.
