...
export default function Home() {
  ...
  const championName = week.teams.reduce((winner, team) => team.points > winner.points ? team : winner, week.teams[0]).name;
      ...
      <main className="main-content">
        ...
        <section className="hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-stat-row">
              <div><strong>{players.length.toString().padStart(2, "0")}</strong><span>jogadores</span></div>
        ...
        <section className="dashboard-grid">
          ...
          <Card className="wheel-card">
            ...
            <div className={`wheel ${isSpinning ? "wheel-spinning" : ""}`}>
              <div className="wheel-segments">{players.map((player) => <span key={player.name}>{player.initials}</span>)}</div>
        ...
        <section className="week-result-section">
          ...
          <div className="teams-grid">
            {week.teams.map((team) => (
                <Card className={`team-card ${team.color} ${team.name === championName ? "winner" : ""}`} key={team.name}>
                <div className="team-topline"><span className="team-dot" /> <span>{team.name}</span>{team.name === championName && <Badge className="champion-badge"><Medal size={13} /> CAMPEÃO</Badge>}</div>
                ...
                {team.name === championName && <div className="winner-shout">A equipa que veio para jogar.</div>}
              </Card>
            ))}
          </div>
